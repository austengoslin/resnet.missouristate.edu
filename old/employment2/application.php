...
body;
	...
	echo	$HTTP_POST_FILES['userfile']['error'];
	
	if (is_uploaded_file($attachment)) { //Do we have a file uploaded?      $fp = fopen($attachment, "rb"); //Open it
      $data = fread($fp, filesize($attachment)); //Read it
	   echo "Filesize: " . filesize($attachment);
	  if ((filesize($attachment)) > 2000000) {
		echo "<div class=\"error\">The attached file must be under 2MB. </div>";
		echo filesize($attachment);
		exit();
	  }
...
if(!isset($hasError)) {
    /* http://php.net/manual/en/function.mail.php says that mail(...) returns true or false
       to tell you if the mail was sent or not */
    /* below this line, mail is trying to get sent */
    $the_mail_was_accepted = mail('resnet@missouristate.edu', $subject, $message, $headers);
    /* did it work? $the_mail_was_accepted knows */
    if($the_mail_was_accepted){
        echo "<div class=\"success\">Your application has been successfully submitted </div>";
    } else {
        echo "<div class=\"error\">Your Application did not get submitted. Try using a smaller resume.</div>";
    }
	} 
	else {
    echo "<!-- error message -->"; 
